...
// import { Rentalpointdet, RentalPoint, RentalPoint2, ParkedVehicleDetails, AvailableBattery, AddedBatteries } from 'src/app/models/rentalPoint';
import { RentalPoint, AvailableBattery, RpId, Rentalpointdet, AddedBatteries,Vehicleinout } from 'src/app/models/franchiseRentalPointModel';
...
@Component({
  selector: 'app-franchise-rental-point-details',
  templateUrl: './franchise-rental-point-details.component.html',
  styleUrls: ['./franchise-rental-point-details.component.scss']
})
export class FranchiseRentalPointDetailsComponent implements OnInit {
 historydisplayedColumns: string[] = [ 'address', 'countries_name','states_name', 'city_name', 'pin',  'commissioning_date', 'max_capacity', 'available_capacity'];
  ...
  // VehicleOutdisplayedColumns: string[] = ['trip_ref_no','vehicle_idnumber','model','chassis_number','booking_time','departure_time','vehicle_type_name'];
  ...
  // vehicleoutdatasource:MatTableDataSource  <Vehicleout>
  rentalPointParkedVehicleDetailsdatasource: MatTableDataSource  <any>;
  ...
  // @ViewChild("vehicleOutpaginator", {static: true}) vehicleOutpaginator: MatPaginator;
  // @ViewChild("vehicleOutsort", {static: true}) vehicleOutsort: MatSort;
  ...
  ngOnInit() {
    ...
    this.subs.add(this.rentalpoint_history$.subscribe(
      (data) => {
        ...
        // this.vehicleoutdatasource = new MatTableDataSource(data.vehicle_out);
        // this.vehicleoutdatasource.paginator = this.vehicleOutpaginator;
        // this.vehicleoutdatasource.sort= this.vehicleOutsort;
